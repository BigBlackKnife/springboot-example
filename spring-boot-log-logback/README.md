...
# 设置日志级别
logging.level.com.blaife=trace
# 设置日志存放目录 相对路径如项目根目录
logging.file.path=log
# 设置文件名称
logging.file.name=springboot-log-slf4j-logback
# 在控制台输出的日志的格式 %d{日期} [线程] 日志级别 哪一个类 日志信息
logging.pattern.console=%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %-5level %logger{50} - %msg%n
# 在指定文件中日志的输出格式
logging.pattern.file=%d{yyyy-MM-dd HH:mm:ss.SSS} === [%thread]=== %-5level %logger{50} - %msg%n
```
基本参数如上所示，这已经可以满足我们基本所有的需求了，

## logback日志配置文件详解 logback-spring.xml
```xml
...
<configuration scan="false" scanPeriod="60 seconds" debug="false">
    ...
    <!-- 定义日志文件名称 -->
    <property name="appName" value="springboot-log-slf4j-logback"></property>
...
　　 -->
    <!-- blaife logger -->
    <logger name="com.blaife" level="trace" />
...
我们一般情况下根本使用不到这个xml配置文件，因为配置文件中的配置信息都可以在application中实现。